describe('Compass Export testcase', () => {
   ...
   cy.get('#username').type('DHWADV01')
        ...
        cy.get('.gwt-SuggestBox').type('ORG_3698455')
        cy.get('.search-field-wrapper > .material-icons').click()
        cy.wait(3000)
        // cy.get('#stibo_tab_Linked_Golden_Customer\(s\) > .tabs-panel-tab-inner > :nth-child(1) > .gwt-InlineLabel').contains('Linked Golden Customer(s)').click()
        // cy.wait(3000)
...
cy.wait(3000);
// if validation is required then we can validate if it is succesfull or not
